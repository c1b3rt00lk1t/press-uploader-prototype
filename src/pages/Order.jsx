...
const Order = () => {
  ...
  const srcFilesObj = srcFiles.reduce(
    (acc, b) => ({ ...acc, [b.id]: b.folder }),
    {}
  );
  ...
  const files = srcFiles.filter(
    (item) =>
      !newOrder
        .flatMap((folder) => folder.files.map((file) => file.id))
        .includes(item.id)
  );
              ...
              onDrop={(ev) => {
                ...
                if (ev.target.className === "orderFolder") {
                  ...
                  if (!folderOfDraggedFile.length) {
                    ...
                    folderOfDraggedFile[0].id !== folderOfDroppedFile[0].id
                  ) {
